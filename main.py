...
def get_image_base64(url):
    """Downloads and embeds images as Base64 strings for offline PDF viewing."""
    if not url: return ""
...
def process_element(element, soup_context):
    """Cleans elements: embeds images, classifies math/diagrams, and scrubs links."""
    for ns in element.find_all("noscript"):
        ns.replace_with(ns.decode_contents())
    
    # Strip blue clickable links but keep text
    ...
    def classify_and_embed(img_tag, source_url):
        ...
        # Decision: Scale up diagrams vs keeping math symbols inline
...
def scrape_topic(page, url):
    """Processes a single topic page with Answer/Explanation logic."""
    try:
        # Strategy: Use 'domcontentloaded' to avoid Render timeout on ad-heavy pages
...
def convert(url: str = Query(...)):
    with sync_playwright() as p:
        ...
        # Real User-Agent bypasses bot-detection that causes 404s on cloud servers
        ...
        try:
            if "#" in url:
                ...
                # Strategy: Swap networkidle for domcontentloaded for cloud stability
                page.goto(base, wait_until="domcontentloaded", timeout=60000)
                
                # Render Fix: Force scroll to end to trigger lazy-loaded chapter headers
                page.evaluate("window.scrollTo(0, document.body.scrollHeight)")
                time.sleep(2)
                
                soup = BeautifulSoup(page.content(), "lxml")
                
                # DEEP FUZZY SEARCH: exact ID -> regex ID -> partial text match
                target = soup.find(id=fragment) or soup.find(id=re.compile(re.escape(fragment)))
                if not target:
                ...
                if not container: raise HTTPException(status_code=404, detail="Chapter list container not found.")
            ...
            style = """<style>
                @page { margin: 10mm; }
                body { font-family: sans-serif; font-size: 8.5pt; line-height: 1.2; color: #111; }
                .topic-header { color: #a00; border-bottom: 1.5px solid #a00; page-break-before: always; margin: 15px 0 5px 0; font-size: 11pt; text-transform: uppercase; }
                .question { font-weight: bold; margin-top: 10px; font-size: 9.2pt; }
                .ans-block { background: #f6fff6; border-left: 4px solid #27ae60; padding: 8px; page-break-inside: avoid; margin: 5px 0; }
                .ans-label { color: #27ae60; font-weight: bold; }
                .diagram { max-width: 95%; display: block; margin: 12px auto; border: 1px solid #ddd; padding: 5px; }
            ...
            # Clean filename for safe HTTP delivery
            clean_name = re.sub(r'[\\/*?:"<>|()]', "", filename).replace(" ", "_")
            return FileResponse(output_path, filename=clean_name)
